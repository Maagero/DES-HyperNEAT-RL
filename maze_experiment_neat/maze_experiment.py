...
def eval_fitness(genome, config, trialSim, time_steps=400):
    ...
    control_net = feed_forward.FeedForwardNetwork.create(genome, config)
    ...
    return fitness, genome.get_nodes_cppn(), genome.get_nodes_cppn()
...
def run_experiment(config_file, maze_env, trial_out_dir, args=None, n_generations=100, silent=False):
    ...
    p = population.Population(config)
    ...
    p.add_reporter(checkpoint.Checkpointer(5, filename_prefix='%s/maze-neat-checkpoint-' % trial_out_dir))
